...
to setup
  ...
  set num-levels ceiling(capacity / ((bays-per-level-side * 2) * aisles))

  let width (aisles * (aisle-width + (bay-width * 2))) ; times 2 since there are 2 bays on each side of an aisle
  let height (buffer-height + (num-levels * (bays-per-level-side + 2)))


  let minpxcor 0
  let maxpxcor width - 1
  let minpycor 0
  let maxpycor height - 1

  set simulated-capacity 0
  ...
  while [current-level > 0] [
    ...
    set current-y (current-y - bays-per-level-side - 2)
